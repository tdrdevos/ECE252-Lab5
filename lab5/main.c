...
int main(int argc, char **argv)
{
    ...
    while ((c = getopt(argc, argv, "t:m:v:")) != -1)
    {
        ...
        case 'm':
            ...
            if (m <= 0)
            {
                fprintf(stderr, "%s: m must be greater than 0\n", argv[0]);
                return -1;
            }
            else if (m > 50)
            {
    ...
    char *seedurl = malloc(256 * sizeof(char));

    strcpy(seedurl, argv[argc - 1]);

    if (!strstr(seedurl, "http://"))
    {
        strcpy(seedurl, SEED_URL);
    }
